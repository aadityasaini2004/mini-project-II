...
public class DepartmentHeadController {
    ...
    // 5. ASSIGN TICKET TO STAFF (Head ticket staff ko dega)
    ...
    public String assignTicket(@PathVariable String ticketId, @PathVariable String staffId) {
        ...
        // 🔥 NAYA CODE: NOTIFICATION LOGIC (Staff ko mail bhejna) 🔥
        try {
            String subject = "🎫 New Task Assigned: " + ticket.getTitle();
            String body = "Hello " + staff.getName() + ",\n\n"
                    + "You have Assigned Work Please Check Dashboard.\n\n"
                    + "📌 Ticket Details:\n"
                    + "- Title: " + ticket.getTitle() + "\n"
                    + "- Description: " + ticket.getDescription() + "\n\n"
                    + "Please check your dashboard and Fix Issue As soon as Possible.\n\n"
                    + "Regards,\nDepartment Head";

            emailService.sendMail(staff.getEmail(), subject, body);
        } catch (Exception e) {
            // Agar mail fail hui toh console me print hoga, par ticket assign ho jayegi
            System.err.println("Email does not successfully!: " + e.getMessage());
        }
